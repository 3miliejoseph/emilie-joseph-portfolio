...
export function ParticlesBackground() {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      // Check if we're in a smaller container (like Passion Projects section)
      const containerHeight = canvas.parentElement?.offsetHeight || window.innerHeight;
      const documentHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight, window.innerHeight);
      
      // Use container height if it's significantly smaller than document height
      H = containerHeight < documentHeight * 0.5 ? containerHeight : documentHeight;
      canvas.height = H;
    }
